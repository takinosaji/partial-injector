...
from sversion.version_file_based import VersionNotFoundException, get_version
...
def test_get_version_found_with_folder_path(
    mock_os_path_exists, mock_open_version_file
):
...
def test_get_version_with_custom_file_name_found(
    mock_os_path_exists, mock_open_custom_version_file
):
    mock_os_path_exists.side_effect = lambda path: path.endswith("CUSTOM_VERSION.txt")
    assert (
        get_version(
            os.path.abspath(os.path.dirname(__file__)),
            version_file_name="CUSTOM_VERSION.txt",
        )
        == "2.0.0"
    )
