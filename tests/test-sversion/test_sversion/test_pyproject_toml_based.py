...
from sversion.pyproject_toml_based import VersionNotFoundException, get_version
...
def mock_open_no_version_section():
    ...
    """
    with patch(
        "builtins.open", mock_open(read_data=no_version_toml_content)
    ) as mock_file:
        yield mock_file
...
def test_get_version_found_with_folder_path(
    mock_os_path_exists, mock_open_pyproject_toml
):
...
def test_get_version_found_with_file_path(
    mock_os_path_exists, mock_open_pyproject_toml
):
    mock_os_path_exists.side_effect = lambda path: path.endswith("pyproject.toml")
    file_path = os.path.join(
        os.path.abspath(os.path.dirname(__file__)), "dummy_file.py"
    )
...
def test_get_version_with_custom_project_file_found(
    mock_os_path_exists, mock_open_custom_project_file
):
    mock_os_path_exists.side_effect = lambda path: path.endswith("custom_project.toml")
    assert (
        get_version(
            os.path.abspath(os.path.dirname(__file__)),
            project_file_name="custom_project.toml",
        )
        == "2.0.0"
    )
...
def test_get_version_found_in_project_section(
    mock_os_path_exists, mock_open_uv_project_file
):
    mock_os_path_exists.side_effect = lambda path: path.endswith("pyproject.toml")
...
def test_get_version_not_found_when_no_known_section(
    mock_os_path_exists, mock_open_no_version_section
):
    mock_os_path_exists.side_effect = lambda path: path.endswith("pyproject.toml")
    with pytest.raises(VersionNotFoundException):
        get_version(os.path.abspath(os.path.dirname(__file__)))
