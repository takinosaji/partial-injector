...
def test_container_can_inject_single_container_in_returns():
    ...
    container.register_singleton(
        [return_one], key=list[NumberReturner], inject_items=True
    )
    container.register_singleton(
        number_adder_factory, key=NumberAdderFactory, inject_returns=True
    )
    container.build()
...
def test_container_can_inject_multiple_containers_in_returns():
    ...
    container.register_singleton(return_constant, key=ConstantReturner)
    container.register_singleton(
        [return_one, return_one], key=list[NumberReturner], inject_items=True
    )
    container.register_singleton(
        number_adder_factory, key=NumberAdderFactory, inject_returns=True
    )
