...
def test_container_can_resolve_subsection():
    configuration = Configuration(
        section=ConfigurationSection(property1="value1", property2="value2")
    )
    container = Container()
    container.register_singleton(configuration, key=Configuration)
    container.register_singleton(
        FromContainer(Configuration, lambda conf: conf.section),
...
def test_from_container_lambda_executed_when_passed_as_parameter():
    ...
    container.register_singleton_factory(
        property_returner,
        factory_args=[
            FromContainer(Configuration, lambda conf: conf.section.property1)
        ],
        key="factory1",
    )
    container.register_singleton_factory(
        property_returner,
        factory_args=[
            FromContainer(Configuration, lambda conf: conf.section.property2)
        ],
        key="factory2",
    )
    container.build()
